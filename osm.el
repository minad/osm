...
(defun osm--tile-as-href (file &optional link)
        ...
        (dotimes (i nx)
          (let* ((x (+ i (/ (- x0 wx) 256)))
                 ...
                 (tile  (and osm--tile-cache (gethash `(,osm-server ,zoom ,x . ,y)
                                                   osm--tile-cache))))
            (if-let (href (plist-get (cddr tile) :file))
              (insert
               (format "<image width=\"256\" height=\"256\" x=\"%d\" y=\"%d\" href=\"%s\"/>\n" (* 256 i)  (* 256 j)
                       (osm--tile-as-href href as-links)))
              (insert (if tile (format "<svg x=\"%d\" y=\"%d\" %s"
                                                (* 256 i)  (* 256 j)
                                                (substring (plist-get (cddr tile) :data) 5))
                                 ""))))))
      (insert "</svg>"))))
